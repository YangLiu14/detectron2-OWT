...
from tracktor_utils import bbox_overlaps, warp_pos, get_center, get_height, get_width, make_pos
...
class Tracker:
	...
	def step(self, blob):
		...
		img = blob['img']  # np.array
		proposals = blob['props']
		dets = [p['bbox'] for p in proposals]
		boxes, scores = list(), list()
		for bbox in dets:
			predictions = self.obj_detect(img, np.array([bbox]))
			box = predictions['instances'].pred_boxes[0].tensor.cpu().numpy().tolist()[0]
			score = predictions['instances'].scores[0].cpu().numpy().tolist()
			bg_score = predictions['instances'].bg_scores[0].cpu().numpy().tolist()
			objectness = predictions['instances'].objectness[0].cpu().numpy().tolist()
			boxes.append(box)
			scores.append(score)
		# TODO: I'm using current image and current detections, which is obviously wrong.
		# boxes, scores
