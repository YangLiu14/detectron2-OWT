...
from kf_tracker import similarity_tracktor_kalman_filter
...
def eval_similarity(predictor, similarity_func: str, datasrc: str, gt_path: str, prop_dir: str, opt_flow_dir: str,
    ...
    num_correct, num_evaled = 8549, 10099
    num_correct_known, num_evaled_known = 6170, 6994
    num_correct_neighbor, num_evaled_neighbor = 815, 912
    num_correct_unknown, num_evaled_unknown = 1564, 2193
    ...
    for vidx, video in enumerate(videos[99:]):
        similarity_record = dict()
        print("{}/{} Process Videos {}/{}".format(vidx + 99, len(videos), datasrc, video))
            ...
            for propL in props_L:
                ...
                if similarity_func == "tracktor-continuous-bbox":
                    ...
                    matched_idx, top_iou = similarity_tracktor(predictor, propL, props_R, frameL, frameR,
                                                               ...
                                                               opt_flow_dir=os.path.join(opt_flow_dir, video),
                                                               mode='bbox', use_frames_in_between=False)
                elif "tracktor-kalman-filter" in similarity_func:
                    iou_thresh = float(similarity_func.split('-')[-1])
                    matched_idx, top_iou = similarity_tracktor_kalman_filter(predictor, propL, props_R, frameL, frameR,
                                                                             os.path.join(image_dir, video),
                                                                             os.path.join(prop_dir, video),
