...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        txt_fname = "../datasets/tao/tao_val_subset.txt"
        with open(txt_fname) as f:
            content = f.readlines()
        video_names_from_txt = [os.path.join(args.input[0], x.strip()) for x in content]
        ...
        if args.video_src_name:  #only process that one video src
            ...
            test = video_names_from_txt[0].split('/')[8]
            video_names_from_txt = [seq for seq in video_names_from_txt if seq.split('/')[8] in video_src_names]
        ...
        for video_src in video_src_names:
            ...
            for path in video_names_from_txt:
                ...
                print("PROCESS VIDEO: {}/{}".format(video_src, video_name))
                ...
                for frame_path in tqdm.tqdm(frames):
                    ...
                    # TODO: the bbox here is dummy, get real bboxes
                    dummy_bboxes = np.array([[10, 20, 30, 40], [50, 60, 70, 80]])
                    predictions = demo.predictor(img, dummy_bboxes)
                    # end_pred = time.time()
